...
class Paddle(Widget):
    ...
    def pop(self, pos):
        ...
        with self.canvas:
            ...
            self.object_on_board = Rectangle(pos=coord, size=self.size)
            self.state = True
...
class PongGame(Widget):
    ...
    #def pop_paddle(self, *args):
        # get random coordinates for the padlle 
        #random_coord = [
            #randint(1, self.x), randint(1, self.y)]

        #self.paddle.pop(random_coord)    
    ...
    def update(self, dt):
        ...
        if self.ball.x > self.width:            
            ...
            #self.paddle_rythme = self.paddle.interval + self.paddle.duration
            #Clock.schedule_interval(
                #self.paddle.remove, self.paddle_rythme / self.running_time_coeff)
        #elif self.turn_counter == self.paddle.interval:
            #self.pop_paddle()
            #Clock.schedule_interval(
                #self.pop_paddle, self.paddle_rythme / self.running_time_coeff)        
...
class HelpScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        self.help_text = '\
        Настройки: \n \
        Скорость мяча: 4 = обычная скорость \n\
        Колличество игроков: \n \
            0: ИИ против ИИ \n \
            1: ИИ против левого игрока \n \
            2: Игрок против игрока \n \
        Сложность ИИ: easy, moderate, hard или insane \n \
        '
